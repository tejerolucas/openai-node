...
export async function makeSnapshotRequest(requestFn, snapshotIndex = 1) {
    if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
        var capturedResponseContent = null;
    ...
    snapshotState._uncheckedKeys.delete(qualifiedSnapshotName);
    ...
    await handleRequest(() => Promise.resolve(new Response(
...
export async function makeStreamSnapshotRequest(requestFn) {
    if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
        var capturedResponseContent = null;
        async function fetch(url, init) {
            const response = await defaultFetch(url, init);
