...
describe('toFile', () => {
    it('throws a helpful error for mismatched types', async () => {
        ...
        toFile({ foo: 'string' })).rejects.toThrowErrorMatchingInlineSnapshot(`"Unexpected data type: object; constructor: Object; props: ["foo"]"`);
        await expect(
        // @ts-expect-error intentionally mismatched type
        toFile(new MyClass())).rejects.toThrowErrorMatchingInlineSnapshot(`"Unexpected data type: object; constructor: MyClass; props: ["name"]"`);
    });
