...
describe('instantiate azure client', () => {
    ...
    describe('defaultHeaders', () => {
        ...
        test('can be removed with `null`', () => {
            ...
            expect(req.headers).not.toHaveProperty('x-my-default-header');
        });
        test('includes retry count', () => {
            ...
            expect(req.headers['x-stainless-retry-count']).toEqual('1');
        });
    });
    describe('defaultQuery', () => {
        test('with null query params given', () => {
            ...
            expect(client.buildURL('/foo', null)).toEqual(`http://localhost:5000/foo?apiVersion=foo&api-version=${apiVersion}`);
        });
        test('multiple default query params', () => {
            ...
            expect(client.buildURL('/foo', null)).toEqual(`http://localhost:5000/foo?apiVersion=foo&hello=world&api-version=${apiVersion}`);
        });
        test('overriding with `undefined`', () => {
            ...
            expect(client.buildURL('/foo', { hello: undefined })).toEqual(`http://localhost:5000/foo?api-version=${apiVersion}`);
        });
    });
    test('custom fetch', async () => {
        const client = new AzureOpenAI({
            ...
            apiVersion,
            fetch: (url) => {
                return Promise.resolve(new Response(JSON.stringify({ url, custom: true }), {
    ...
    test('custom signal', async () => {
        const client = new AzureOpenAI({
            ...
            apiVersion,
            fetch: (...args) => {
                return new Promise((resolve, reject) => setTimeout(() => defaultFetch(...args)
                    .then(resolve)
                    .catch(reject), 300));
            },
        });
    ...
    describe('baseUrl', () => {
        test('trailing slash', () => {
            ...
            expect(client.buildURL('/foo', null)).toEqual(`http://localhost:5000/custom/path/foo?api-version=${apiVersion}`);
        });
        ...
        test('empty baseUrl/endpoint env variable', () => {
            ...
            expect(() => new AzureOpenAI({ apiKey: 'My API Key', apiVersion })).toThrow(/Must provide one of the `baseURL` or `endpoint` arguments, or the `AZURE_OPENAI_ENDPOINT` environment variable/);
        });
    ...
    describe('Azure Active Directory (AD)', () => {
        test('with azureADTokenProvider', async () => {
            const testFetch = async (url, { headers } = {}) => {
            ...
            expect((await client.request({ method: 'post', path: 'https://example.com' }).asResponse()).headers.get('authorization')).toEqual('Bearer my token');
        });
        test('apiKey and azureADTokenProvider cant be combined', () => {
            expect(() => new AzureOpenAI({
                ...
                apiKey: 'My API Key',
                apiVersion,
            })).toThrow(/The `apiKey` and `azureADTokenProvider` arguments are mutually exclusive; only one can be passed at a time./);
        });
        test('AAD token is refreshed', async () => {
            let fail = true;
            const testFetch = async (url, req) => {
                if (fail) {
                    fail = false;
                ...
                return new Response(JSON.stringify({ auth: (req?.headers)['authorization'] }), { headers: { 'content-type': 'application/json' } });
            };
            let counter = 0;
            ...
            expect(await client.chat.completions.create({
                model,
...
describe('azure request building', () => {
    ...
    describe('model to deployment mapping', function () {
        const testFetch = async (url, { body } = {}) => {
        ...
        describe('with client-level deployment', function () {
            ...
            test('handles completions', async () => {
                ...
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/completions?api-version=${apiVersion}`);
            });
            test('handles chat completions', async () => {
                ...
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/chat/completions?api-version=${apiVersion}`);
            });
            test('handles embeddings', async () => {
                ...
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/embeddings?api-version=${apiVersion}`);
            });
            test('handles audio translations', async () => {
                const { url } = (await client.audio.translations.create({
                    model,
                    file: { url: 'https://example.com', blob: () => 0 },
                }));
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/audio/translations?api-version=${apiVersion}`);
            });
            test('handles audio transcriptions', async () => {
                ...
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/audio/transcriptions?api-version=${apiVersion}`);
            });
            test('handles text to speech', async () => {
                const { url, body } = await (await client.audio.speech.create({
                    model,
                    input: '',
                    voice: 'alloy',
                })).json();
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/audio/speech?api-version=${apiVersion}`);
            ...
            test('handles image generation', async () => {
                ...
                expect(url).toStrictEqual(`https://example.com/openai/deployments/${deployment}/images/generations?api-version=${apiVersion}`);
            });
    ...
    describe('Content-Length', () => {
        test('handles multi-byte characters', () => {
            ...
            expect(req.headers['content-length']).toEqual('20');
        });
        test('handles standard characters', () => {
            const { req } = client.buildRequest({ path: '/foo', method: 'post', body: { value: 'hello' } });
            expect(req.headers['content-length']).toEqual('22');
        });
    });
    describe('custom headers', () => {
        test('handles undefined', () => {
            ...
            expect(req.headers['x-foo']).toEqual('bar');
            expect(req.headers['x-Foo']).toEqual(undefined);
            expect(req.headers['X-Foo']).toEqual(undefined);
            expect(req.headers['x-baz']).toEqual(undefined);
        });
    });
});
describe('retries', () => {
    test('retry on timeout', async () => {
        let count = 0;
        const testFetch = async (url, { signal } = {}) => {
            if (count++ === 0) {
                return new Promise((resolve, reject) => signal?.addEventListener('abort', () => reject(new Error('timed out'))));
            }
        ...
        expect(await client
            ...
            .asResponse()
            .then((r) => r.text())).toEqual(JSON.stringify({ a: 1 }));
