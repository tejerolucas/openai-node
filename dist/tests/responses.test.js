...
describe('response parsing', () => {
    ...
    test('duplicate headers are concatenated', () => {
        const headers = createResponseHeaders(new Headers([
...
describe('request id', () => {
    ...
    test('withResponse', async () => {
        const client = new OpenAI({
            apiKey: 'dummy',
            fetch: async () => new Response(JSON.stringify({ id: 'bar' }), {
                headers: { 'x-request-id': 'req_id_xxx', 'content-type': 'application/json' },
            }),
        });
        const { data: completion, response, request_id, } = await client.chat.completions.create({ messages: [], model: 'gpt-4' }).withResponse();
    ...
    test('envelope response', async () => {
        const promise = new APIPromise((async () => {
            return {
    ...
    test('page response', async () => {
        const client = new OpenAI({
            apiKey: 'dummy',
            fetch: async () => new Response(JSON.stringify({ data: [{ foo: 'bar' }] }), {
                headers: { 'x-request-id': 'req_id_xxx', 'content-type': 'application/json' },
            }),
        });
        const page = await client.fineTuning.jobs.list();
        expect(page.data).toMatchObject([{ foo: 'bar' }]);
        expect(page._request_id).toBeUndefined();
    });
    test('array response', async () => {
        ...
        expect(rsp._request_id).toBeUndefined();
    });
    test('string response', async () => {
        ...
        expect(result._request_id).toBeUndefined();
    });
});
