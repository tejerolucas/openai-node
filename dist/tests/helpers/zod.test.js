...
describe('zodResponseFormat', () => {
    it('does the thing', () => {
        expect(zodResponseFormat(z.object({
            city: z.string(),
            temperature: z.number(),
            units: z.enum(['c', 'f']),
        }), 'location').json_schema).toMatchInlineSnapshot(`
      {
        "name": "location",
        "schema": {
    ...
    it('allows description field to be passed in', () => {
        expect(zodResponseFormat(z.object({
            city: z.string(),
        }), 'city', { description: 'A city' }).json_schema).toHaveProperty('description', 'A city');
    });
    ...
    it('warns on optional fields', () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Zod field at `#/definitions/schema/properties/optional` uses `.optional()` without `.nullable()` which is not supported by the API. See: https://platform.openai.com/docs/guides/structured-outputs?api-mode=responses#all-fields-must-be-required\nThis will become an error in a future version of the SDK.');
    ...
    it('warns on nested optional fields', () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Zod field at `#/definitions/schema/properties/foo/properties/bar/items/properties/can_be_missing` uses `.optional()`'));
        expect(consoleSpy).toHaveBeenCalledTimes(1);
    });
