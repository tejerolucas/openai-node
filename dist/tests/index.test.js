...
describe('retries', () => {
    ...
    test('retry count header', async () => {
        ...
        expect(await client.request({ path: '/foo', method: 'get' })).toEqual({ a: 1 });
        expect(capturedRequest.headers['x-stainless-retry-count']).toEqual('2');
        expect(count).toEqual(3);
    });
    test('omit retry count header', async () => {
        ...
        expect(capturedRequest.headers).not.toHaveProperty('x-stainless-retry-count');
    });
    ...
    test('overwrite retry count header', async () => {
        ...
        expect(capturedRequest.headers['x-stainless-retry-count']).toBe('42');
    });
...
describe('debug()', () => {
    ...
    test('input args are not mutated', function () {
        ...
        const { req } = client.buildRequest({ path: '/foo', method: 'post' });
        debug('request', authorizationTest);
        expect(req.headers['authorization']).toEqual('fakeValue');
        expect(spy).toHaveBeenCalledWith('OpenAI:DEBUG:request', {
            authorization: 'REDACTED',
        });
    });
