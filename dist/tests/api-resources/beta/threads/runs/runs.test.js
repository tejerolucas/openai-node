...
describe('resource runs', () => {
    ...
    test('retrieve: request options instead of params are passed correctly', async () => {
        ...
        await expect(client.beta.threads.runs.retrieve('thread_id', 'run_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('update', async () => {
    ...
    test('list: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.beta.threads.runs.list('thread_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('list: request options and params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.beta.threads.runs.list('thread_id', { after: 'after', before: 'before', limit: 0, order: 'asc' }, { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('cancel', async () => {
    ...
    test('cancel: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.beta.threads.runs.cancel('thread_id', 'run_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
