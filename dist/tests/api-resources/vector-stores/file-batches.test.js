...
describe('resource fileBatches', () => {
    ...
    test('retrieve: request options instead of params are passed correctly', async () => {
        ...
        await expect(client.vectorStores.fileBatches.retrieve('vs_abc123', 'vsfb_abc123', {
    ...
    test('cancel: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.vectorStores.fileBatches.cancel('vector_store_id', 'batch_id', {
            path: '/_stainless_unknown_path',
        })).rejects.toThrow(OpenAI.NotFoundError);
    });
    ...
    test('listFiles: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.vectorStores.fileBatches.listFiles('vector_store_id', 'batch_id', {
            path: '/_stainless_unknown_path',
        })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('listFiles: request options and params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.vectorStores.fileBatches.listFiles('vector_store_id', 'batch_id', { after: 'after', before: 'before', filter: 'in_progress', limit: 0, order: 'asc' }, { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
});
