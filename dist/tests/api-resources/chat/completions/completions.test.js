...
describe('resource completions', () => {
    ...
    test('retrieve: request options instead of params are passed correctly', async () => {
        ...
        await expect(client.chat.completions.retrieve('completion_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    ...
    test('list: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.chat.completions.list({ path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('list: request options and params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.chat.completions.list({ after: 'after', limit: 0, metadata: { foo: 'string' }, model: 'model', order: 'asc' }, { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
    test('del', async () => {
    ...
    test('del: request options instead of params are passed correctly', async () => {
        // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
        await expect(client.chat.completions.del('completion_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(OpenAI.NotFoundError);
    });
});
